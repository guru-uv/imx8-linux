			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		dev_err(component->dev, "%s: turned on\n", __func__);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* turn mic bias off */
		snd_soc_component_update_bits(component, AIC31XX_MICBIAS,
				    AIC31XX_MICBIAS_MASK, 0);
		dev_err(component->dev, "%s: turned off\n", __func__);
		break;
	}
	return 0;
}
			     ...
			     struct snd_soc_dai *dai)
{
	...
	dev_err(component->dev, "## %s: width %d rate %d\n",
				   ...
				   unsigned long event, void *data)
{
	...
	if (event & REGULATOR_EVENT_DISABLE) {
		...
		dev_err(aic31xx->dev, "## %s: DISABLE received\n", __func__);
	}
...
static void aic31xx_clk_on(struct snd_soc_component *component)
{
	...
	dev_err(component->dev, "codec clock -> on (rate %d)\n",
...
static void aic31xx_clk_off(struct snd_soc_component *component)
{
	...
	dev_err(component->dev, "codec clock -> off\n");
